	...
	return result.match({
		ok: (parsed) => {
			...
			return {
				...
				globalPatterns: toArray(config.globalPatterns).map((spec) =>
					compilePattern(spec),
				),
			};
		},
...
export function collectRules(
	...
	const candidates =
		type === "read" ? getPathCandidates(subject, cwd) : [subject];
	...
	pi.on("tool_result", async (event, ctx) => {
		...
		if (event.toolName === "read") {
			...
			return maskContent(event.content, (text) =>
				applyRules(text, rules, state.config),
			);
		}

		if (event.toolName === "bash") {
			const command =
				"command" in event.input && typeof event.input["command"] === "string"
					? event.input["command"]
					: "";
