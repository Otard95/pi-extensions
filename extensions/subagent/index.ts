		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
					...
					content: [
						{
							type: "text",
							text:
								getFinalOutput(at(results, results.length - 1).messages) ||
								"(no output)",
						},
					],
					details: makeDetails("chain")(results),
			...
			if (params.tasks && params.tasks.length > 0) {
				...
				for (let i = 0; i < params.tasks.length; i++) {
					allResults[i] = {
						agent: at(params.tasks, i).agent,
						agentSource: "unknown",
						task: at(params.tasks, i).task,
